...
A chat session requires two pieces of shared secret, generated by the initiator:
...
Messages are organized in a **linked chain of groups**. Each group is owned by one party (alternating: initiator, responder, initiator, …).
...
```
 initial_prefix = SHA256(secret_id)[0..15]
       ...
       ▼
 ┌─────────────────────────────────────────┐
 │  Group 0  (initiator owns this group)   │
 │  prefix = initial_prefix                │
 │                                         │
 │  DB entries with this prefix:           │
 │  ├── Enc(FID₀)         ← forward ID    │
 │  ├── Enc(eid + msg_A)  ← message       │
 │  └── Enc(eid + msg_B)  ← message       │
 └──────────────────┬──────────────────────┘
                    │  SHA256(FID₀)[0..15]
                    ▼
 ┌─────────────────────────────────────────┐
 │  Group 1  (responder owns this group)   │
 │  prefix = SHA256(FID₀)[0..15]           │
 │                                         │
 │  DB entries with this prefix:           │
 │  ├── Enc(FID₁)         ← forward ID    │
 │  └── Enc(eid + msg_C)  ← message       │
 └──────────────────┬──────────────────────┘
                    │  SHA256(FID₁)[0..15]
                    ▼
 ┌─────────────────────────────────────────┐
 │  Group 2  (initiator owns this group)   │
 │  …                                      │
 └─────────────────────────────────────────┘
```
...
```
 ...
 ───────   ───────   ───────   ───────   ───────
 Initiator Responder Initiator Responder Initiator
...
```
  state = 0 (uninit)
      │
      ├──► proto_initialize() ──► state = 1 (can_send)   [initiator]
      └──► proto_join()       ──► state = 2 (need_list)  [responder, waits for chain]

  state = 2 ──► proto_list() walks chain ──► state = 1 if responder's turn found
...
```
 ...
 Not protected: traffic analysis (file growth rate, timing)
               ...
               multi-party confidentiality (all participants on a side share the same key)
...
int main(void)
{
    ...
    /* Send a message */
    proto_send(&chat, "Hello from initiator!");
...
const proto_messages *msgs = proto_list(&chat);
/* After the initiator's group appears, state becomes 1 and the responder can send */
if (chat.state == 1)
    proto_send(&chat, "Hello back!");
