...
```
 ...
 secret_id (32 hex chars, 16 bytes random)  ← also used as KDF salt
     │
     ├──► SHA256^KDF_ROUNDS( SHA256("aes:"  + user_key + ":" + secret_id) ) = aes_key  [32 bytes]
     ├──► SHA256^KDF_ROUNDS( SHA256("hmac:" + user_key + ":" + secret_id) ) = hmac_key [32 bytes]
     └──► SHA256(secret_id_hex_string)[0..15]  = initial_prefix [32 hex chars]
```

**Slow KDF:** both `aes_key` and `hmac_key` are derived by chaining SHA-256 `KDF_ROUNDS` times (default 2 000 000, ≈ 5 s on typical hardware). This means every credential guess an attacker makes costs ~5 s of computation, making offline brute-force impractical even if `messages.db` is stolen.

`secret_id` is used as a per-chat **salt** in the KDF, so precomputed tables for one chat cannot be reused against another — even if the same `user_key` were used twice.

The `initial_prefix` is the entry point of the message chain. Knowing only the prefix does not reveal either key.
...
```
 ...
 Protected:   message content (AES-256-CBC)
              ...
              offline brute-force (~5 s per guess via slow KDF)
...
**Key derivation / brute-force resistance:** both AES and HMAC keys are derived by iterating SHA-256 `KDF_ROUNDS` times (default 2 000 000, ≈ 5 s). An attacker who obtains `messages.db` and attempts to guess credentials pays ~5 s per guess. `secret_id` is used as a per-chat salt so no guess work can be reused across chats.

**Key separation:** separate keys for AES and HMAC, derived with domain prefixes (`"aes:"` / `"hmac:"`) and independent iteration chains.
...
| `KDF_ROUNDS` | 2 000 000 | SHA-256 iterations in key derivation; ≈ 5 s per join/load on typical hardware. Override with `-DKDF_ROUNDS=N` at compile time. |
